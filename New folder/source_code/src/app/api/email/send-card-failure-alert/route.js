...
// GET /api/email/send-card-failure-alert - Check email configuration
...
// POST /api/email/send-card-failure-alert - Send card failure alert email to a donor
export async function POST(request) {
  try {
    ...
    if (emailResult.success) {
      return NextResponse.json({
        success: true,
        message: 'Card failure alert email sent successfully',
        emailResult: {
      ...
      console.error('Failed to send card failure alert email:', emailResult.error);
      return NextResponse.json(
        { success: false, error: 'Failed to send card failure alert email', details: emailResult.error },
        { status: 500 }
      );
    }
    ...
    return NextResponse.json(
      { success: false, error: 'Failed to send card failure alert email', details: error.message },
      { status: 500 }
    );
  }
}
