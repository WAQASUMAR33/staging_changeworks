...
export async function POST(request) {
  try {
    ...
    const transaction = await prisma.saveTrRecord.create({
      data: {
        ...
        trx_details: JSON.stringify({
          ...
          created_with_auto_sync: true
        }),
        pay_status: 'pending'
      }
    });

    // Schedule automatic status check after 30 seconds
    // This simulates what would happen when the user completes the payment
    setTimeout(async () => {
      try {
        ...
        if (updatedStripePayment.status === 'succeeded') {
          ...
          console.log(`✅ Auto-synced payment ${paymentIntent.id} to completed status`);
        }
      } catch (error) {
        console.error(`❌ Error auto-syncing payment ${paymentIntent.id}:`, error);
      }
    ...
    return NextResponse.json({
      ...
      message: "Payment intent created successfully with auto-sync enabled",
      auto_sync_note: "Payment status will be automatically checked and updated after completion"
    });
